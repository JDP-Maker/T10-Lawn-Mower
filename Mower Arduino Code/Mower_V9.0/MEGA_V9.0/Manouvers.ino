...
void Manouver_Find_Wire_Track()  {
	...
	message_out.println(F("Find Wire Track Function Activated"));
	lcd.clear();
	...
	for (int i = 0; i <= 1; i++) {
		if (Mower_Parked == 0) {
			...
			UpdateWireSensor();
			delay(20);
			message_out.println(F("before if loops start"));
	...
	while (( inside == false)  && (Abort_Wire_Find == 0) && (No_Wire_Found_Fwd == 0) && (Mower_Parked ==0) && (Spin_Attempts < Max_Spin_Attempts )) {
        while (( inside != true) && (Spin_Attempts < Max_Spin_Attempts )) {                                                             // Do this loop until mower is back  the wire fence
			message_out.print(F("Spin Attempts"));
...
void Manouver_Turn_Around() {
    ...
    if (GPS_Inside_Fence == 0) message_out.println(F("Mower is Outside the GPS Fence"));
    ...
    if (TFT_Screen_Menu == 1)        Send_Mower_Running_Data();           // Update TFT Screen
    message_out.println(F(""));
    message_out.println(F("Mower Turned Around"));
    message_out.println(F(""));
}
...
//Added by JDP ***********************************************************************************
void Manouver_Turn_Around_Rear_Sonar() {
	Sonar_Status = 1;
	message_out.println(F("Mower is Turning - Sonar"));
	Motor_Action_Stop_Motors(); 
	...
	if ( distance4 < maxdistancesonar ||  distance5 < maxdistancesonar ) {
		lcd.setCursor(0, 8);
		lcd.print("Go Right -->       ");
		SetPins_ToTurnRight();
		Motor_Action_Turn_Speed();
...
void Manouver_Exit_To_Zone_X() {
	...
	if (Perimeter_Wire_Enabled == 1) {
		...
		//          message_out.println(F(""));
		//          message_out.println("Perimeter Wire not detected");
		//          Manouver_Park_The_Mower();
